...
def insertStorageFileUsingConfig (config_file, filepath, schema = 'storage', plates_schema = 'plates', boxes_schema = 'boxes'):
	...
	plate_update_dataframe = plate_update_dataframe.replace('', np.nan)
	...
	for update_dict in plate_update_dataframe.to_dict('records'):
		...
		#plate_update.update()
		plate_updates.append(plate_update)
	...
	box_update_dataframe = box_update_dataframe.replace('', np.nan)
	...
	for update_dict in box_update_dataframe.to_dict('records'):
		...
		#box_update.update()
		box_updates.append(box_update)

	plate_insert.insertIgnore()
	box_insert.insertIgnore()
	for plate_update in plate_updates: plate_update.update()
	for box_update in box_updates: box_update.update()
